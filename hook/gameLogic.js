...
const drawAll = function (ws, gameInstance) {
    drawPlayerTank(ws, gameInstance);
...
const addEnemyTank = function (ws, gameInstance) {
    ...
    ServerSendMsg(
        ...
        new SyncMsg('enemy_add', SYNC_SERVER_TYPE.ENEMYTANK_ADD, { tankType: obj.tankType, x: obj.x })
    );
    //更新地图右侧坦克数
...
//游戏结束
const gameOver = function (gameInstance) {
    //gameInstance.overCtx.clearRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    const { overX, overY } = gameInstance
    const mapOffsetX = gameInstance.map.offsetX;
    const mapOffsetY = gameInstance.map.offsetY;
    let refers = { overX, overY, mapOffsetX, mapOffsetY }
    ServerSendMsg(
        ws,
        MSG_TYPE_SERVER.MSG_OPERA_DRAW,
        new DrawMsg('over_draw', OPERA_DRAW_TYPE.OVER_DRAW, refers)
    );
